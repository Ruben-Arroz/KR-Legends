...
try {
    $stmt = $mysqli->prepare("
        SELECT 
            ...
            GL_CREATED_AT
        FROM KR_GALERIA 
        WHERE GL_IS_PUBLIC = 1 
        ORDER BY GL_CREATED_AT DESC
    ");
    $stmt->execute();
    $result = $stmt->get_result();

    while ($row = $result->fetch_assoc()) {
        // Determinar extensão do arquivo (verificar se existe)
        $extensions = ['jpg', 'jpeg', 'png', 'gif', 'mp4', 'webp'];
        $imagePath = '';

        foreach ($extensions as $ext) {
            $filePath = "Imagens/Galeria/item_{$row['GL_ID']}.{$ext}";
        ...
        if ($imagePath) {
            $galleryItems[] = [
                'id' => $row['GL_ID'],
                'title' => $row['GL_TITLE'],
                'description' => $row['GL_DESCRIPTION'],
                'category' => $row['GL_CATEGORY'],
                'alt' => $row['GL_ALT'] ?: $row['GL_TITLE'],
                'imagePath' => $imagePath,
                'created_at' => $row['GL_CREATED_AT']
            ];
        }
    }
    $stmt->close();
...
<head>
    ...
    <title>KR Legends - Galeria</title>
    ...
    <script type="module">
        import Muuri from "https://cdn.jsdelivr.net/npm/muuri@0.9.5/dist/muuri.module.js";
    </script>
    <script>
        // Passar dados PHP para JavaScript
        window.galleryData = <?php echo json_encode($galleryItems); ?>;
    </script>
...
<body class="bg-black text-white">
    ...
    <section id="hero" class="min-vh-100 d-flex align-items-center text-center position-relative overflow-hidden">
        ...
        <div class="container position-relative hero-content">
            <h1 class="display-2 fw-bold text-warning mb-4 fade-in" data-translate="hero.pitstop">A Nosssa Galeria</h1>
            <p class="lead text-white mb-5 mx-auto slide-up" style="max-width: 600px;" data-translate="hero.subtitle">
                Aqui você verá registros dos melhores momentos e melhores paisagens do jogo <b>KR Legends</b>.
            </p>
